...
def send_email_about_update_course(course_id):
    ...
    subscribers = Subscription.objects.filter(course=course).select_related("users")
    ...
    email_list = [sub.user.email for sub in subscribers]
    ...
    if email_list:
        ...
        course.last_updated = timezone.now()  # для сброса начала отсчета четырех часов
        course.save()
